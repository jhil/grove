...
function calculateUrgency(plant: Plant): { status: string; daysOverdue: number } {
...
export function registerPlantTools(server: McpServer): void {
        ...
        content: [
          {
            type: "text",
            text:
              output.plants.length > 0
                ? output.plants
                    .map(
                      (p) =>
                        `- ${p.name} (${p.type}) [${p.status}] - streak: ${p.streakCount}`
                    )
                    .join("\n")
  ...
  server.registerTool(
    ...
    async ({ plantId, name, type, wateringInterval, notes }) => {
      ...
      if (wateringInterval !== undefined) updates.watering_interval = wateringInterval;
