...
import type { Grove } from "../types.js";
...
function generateGroveId(name: string): string {
  const slug = name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "")
...
export function registerGroveTools(server: McpServer): void {
  // List groves
  server.registerTool(
    "list_groves",
    {
    ...
    async () => {
      ...
      const output = {
        groves: (groves || []).map((g: Grove & { plants: { count: number }[] }) => ({
          id: g.id,
          name: g.name,
  ...
  server.registerTool(
    ...
    async ({ groveId, name, coverPhoto }) => {
      ...
      if (coverPhoto) updates.cover_photo = coverPhoto;
  ...
  server.registerTool(
    ...
    async ({ groveId, confirm }) => {
      ...
      if (!confirm) {
        throw new Error(
          "Deletion not confirmed. Set confirm=true to delete the grove and all its plants."
        );
      }
